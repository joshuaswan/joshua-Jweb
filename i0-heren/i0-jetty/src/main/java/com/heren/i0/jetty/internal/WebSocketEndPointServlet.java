...
public class WebSocketEndPointServlet extends HttpServlet {
    ...
    public void init() throws ServletException {
        try {
            ...
            for (String value : fromNullable(getInitParameter("bufferSize")).asSet())
                policy.setInputBufferSize(Integer.parseInt(value));
            ...
            factory.init(super.getServletContext());
        } catch (Exception x) {
    ...
    private class SessionAndMessageMethod {
        ...
        public SessionAndMessageMethod(Class<?> aClass) {
            method = tryFind(copyOf(aClass.getMethods()), annotateWith(OnWebSocketMessage.class));
        }
    ...
    public class EndPointAdapter extends WebSocketAdapter {
        ...
        public EndPointAdapter(Object socket) {
            this.socket = socket;
            webSocketOpen = new SessionMethod(OnWebSocketConnect.class, socket.getClass());
            webSocketClose = new SessionMethod(OnWebSocketClose.class, socket.getClass());
