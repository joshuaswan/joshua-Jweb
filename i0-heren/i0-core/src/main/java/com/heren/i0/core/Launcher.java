...
import com.heren.core.core.internal.logging.Logging;
import com.heren.core.core.internal.util.ClassScanner;
...
import static com.heren.core.core.internal.util.TypePredicates.isApplicationModule;
import static com.heren.core.core.internal.util.TypePredicates.moduleName;
...
public class Launcher {
    ...
    public static ServletContainer launch(ApplicationModule<? extends Configuration> module, boolean standalone) throws Exception {
        ...
        Map.Entry<Annotation, FacetEnabler> enablerWebsocket = null;
        for (Map.Entry<Annotation, FacetEnabler> enabler : module.enablers().entrySet()) {
            if (enabler.getValue() instanceof ContainerConfigurator) {
                ContainerConfigurator containerConfigurator = (ContainerConfigurator) enabler.getValue();
                String name = containerConfigurator.getClass().getSimpleName();
                if (name.equals("WebSocketEnabler")) {
                    enablerWebsocket = enabler;
                } else {
                    containerConfigurator.configure(container, enabler.getKey(), module);
                }
            }
        }

        if(enablerWebsocket != null){
            ((ContainerConfigurator) enablerWebsocket.getValue()).configure(container, enablerWebsocket.getKey(), module);
        }
    ...
    public static ServletContainer launchForSchedule(String... arguments) throws Exception {
        checkArgument(arguments.length <= 2, "Too many arguments");
        if(arguments.length == 2){
            configFolder = arguments[1];
        }
        return launch(readConfiguration(arguments.length == 0 ? findDefaultApplicationModule() : findApplicationModule(arguments[0])), false);
    }
