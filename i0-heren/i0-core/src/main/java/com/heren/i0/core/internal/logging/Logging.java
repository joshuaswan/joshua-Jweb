package com.heren.core.core.internal.logging;
...
import com.heren.core.config.LoggingConfiguration;
...
import static com.heren.core.config.LoggingConfiguration.ConsoleConfiguration;
import static com.heren.core.config.LoggingConfiguration.FileConfiguration;
...
//        appender.setFile(file.getFilename());
        return rollingFileAppender(file, context);
    }

    private static FileAppender<ILoggingEvent> rollingFileAppender(FileConfiguration file, LoggerContext context) {
        ...
        if (file.getArchive().isPresent()) {
            triggeringPolicy.setMaxFileSize(file.getArchive().get().getMaxFileSize().toString());
        }
        ...
        if (file.getArchive().isPresent()) {
            rollingPolicy.setFileNamePattern(file.getArchive().get().getNamePattern());
        } else {
            rollingPolicy.setFileNamePattern(file.getFilename());
        }
        rollingPolicy.setTimeBasedFileNamingAndTriggeringPolicy(triggeringPolicy);
        triggeringPolicy.setTimeBasedRollingPolicy(rollingPolicy);
        if (file.getArchive().isPresent()) {
            rollingPolicy.setMaxHistory(file.getArchive().get().getMaxHistory());
            rollingPolicy.setCleanHistoryOnStart(file.getArchive().get().getCleanHistoryOnStart());
        }
